    ...
    def completed(self, had_error=None):
        if had_error:
            ...
            for objid, objname in zip(self.object_list, self.object_names):
                ...
                object_type.append(objid[0])
                object_id.append(objid[1])
...
class ReadObjectListApplication(BIPSimpleApplication):
    ...
    def read_object_list(self, device_id, device_addr):
        ...
        try:
            ...
            err = f"Error on read_object_list - {error}"
            logging.error(err)
            stop()
    ...
    def object_list_results(self, iocb):
        ...
        try:
            ...
            err = f"Error on object_list_results - {error}"
            logging.error(err)
            stop()
    ...
    def read_next_object(self, context):
        ...
        try:
            ...
            err = f"Error on read_next_object - {error}"
            logging.error(err)
            stop()
    ...
    def object_name_results(self, iocb):
        ...
        try:
            ...
            err = f"Error on object_name_results - {error}"
            logging.error(err)
            stop()
...
def main():
    ...
    logging.basicConfig(filename=f'bacpypes_logs/{args.device_id}.log', level=logging.ERROR,filemode='a',format= '%(asctime)s - %(levelname)s: %(message)s',\
